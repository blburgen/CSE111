...
from waterflow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe,\
    pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, kpa_psi
from pytest import approx
import pytest

def test_kpa_psi():
    assert kpa_psi(0) == approx(0.0, abs=0.1)
    assert kpa_psi(158.7) == approx(23.0, abs=0.1)
    assert kpa_psi(30.0) == approx(4.4, abs=0.1)
    assert kpa_psi(1589.6) == approx(230.6, abs=0.1)
